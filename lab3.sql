...
SELECT code FROM departments GROUP BY code HAVING count(*) > 2;
...
UPDATE departments SET budget = budget - budget * 0.1 WHERE budget = (SELECT budget FROM departments ORDER BY budget ASC LIMIT 1) 
RETURNING *;
